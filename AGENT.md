...
**DormPlus** — เว็บแอปจัดการหอพักภาษาไทยสำหรับเจ้าของหอ
...
1. Scaffold Next.js + TypeScript + Tailwind + shadcn/ui, ตั้ง design token ตาม DESIGN.md §8, โหลดฟอนต์ Prompt
2. เขียน Prisma schema ตาม SPEC.md §4 → migration → `seed.ts` ตาม §8
3. เขียน `lib/format.ts` + unit test (SPEC.md §9)
4. เขียน `lib/kpi.ts` + unit test (SPEC.md §5) — ต้องได้ ↑12%, ↑5%, ↑18% จาก seed
5. เขียน `lib/dashboard.ts` และ `app/api/dashboard/summary/route.ts`
6. สร้าง layout: `Sidebar`, `Topbar`
7. สร้างการ์ดทีละตัวตาม DESIGN.md §4.3–4.11 แต่ละตัวมี loading / empty / error
8. ประกอบหน้า `app/(dashboard)/page.tsx` ด้วย grid ตาม DESIGN.md §3
