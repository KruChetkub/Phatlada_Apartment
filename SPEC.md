# SPEC.md — DormPlus (ระบบจัดการหอพัก)
...
เจ้าของหอพักเปิดหน้าหลักแล้วตอบได้ภายใน 10 วินาทีว่า:
...
| Phase 2 | CRUD: ห้องพัก, ผู้เช่า, สัญญาเช่า, การชำระเงิน, รายจ่าย, แจ้งซ่อม |
| Phase 3 | รายงาน, ข้อความ, การแจ้งเตือนแบบ real-time, ตั้งค่า, หลายหอพักต่อบัญชี |
...
| ORM | Prisma |
| Validation | Zod |
| Auth | Auth.js (NextAuth) หรือ Supabase Auth |
...
├─ components/
│  ├─ layout/        Sidebar.tsx, Topbar.tsx, DormCard.tsx
...
│  └─ dashboard.ts   # query รวมข้อมูล dashboard
├─ types/
prisma/
├─ schema.prisma
└─ seed.ts
...
model User {
  ...
  dormitories DormitoryMember[]
}

model Dormitory {
  id        String   @id @default(cuid())
  name      String                        // "หอพักสุขสันต์"
...
| แจ้งเตือน | Notification ของหอ | 4 | `createdAt` desc |
...
### `GET /api/dashboard/summary?dormitoryId={id}`

- ต้องล็อกอิน และผู้ใช้ต้องเป็นสมาชิกของหอนั้น (ไม่ใช่ → `403`)
- เงินใน response เป็น **บาท (จำนวนเต็ม)** แปลงจากสตางค์ที่ชั้น API
- วันเวลาเป็น ISO 8601 (UTC) — ฝั่ง UI แปลงเป็นไทย

```json
{
  "dormitory": { "id": "d_1", "name": "หอพักสุขสันต์", "plan": "PREMIUM" },
...
หน้า Dashboard เป็น Server Component ที่เรียก `lib/dashboard.ts` โดยตรงได้ ส่วน API route มีไว้ให้ client/mobile ใช้ ทั้งสองต้องใช้ฟังก์ชันเดียวกัน

---

## 8. Seed Data (ตรงกับภาพดีไซน์)

- หอพัก: **หอพักสุขสันต์** (PREMIUM)
- ผู้ใช้: คุณนท ถาวร ศรีเสนพิลา — เจ้าของหอพัก
...
- [ ] หน้า `/` แสดงผลใกล้เคียง `assets/reference-dashboard.png` ที่ความกว้าง 1536px
- [ ] ตัวเลขทุกตัวมาจาก DB ไม่มีการ hard-code ในคอมโพเนนต์
